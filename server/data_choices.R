...
#     need(input$regnat, label = "all countries in region")
#   )
#   
#   n1<-NULL
#   
...
#   if(ind_sex$sex == 0)
#     ch <- sex1[1]
#   if(ind_sex$name %in% c("assr","eassr"))
#     ch <- sex1[-1]
...
# 
# year_label <- reactive({
#   ifelse(year_period() == 1, "Period", "Year")
# })
# 
...
#   validate(
#     need(input$allyear, label = "all year tick box")
#   )
#   s <- NULL
#   s <- yn0[14][[1]]
#   if(year_past() == 1)
...
# 
# observe({
#   updateSelectizeInput(session, inputId = "year", label = year_label(), choices = year_choice(), selected = year_select())
# })
#
